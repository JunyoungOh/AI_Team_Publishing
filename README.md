...
> **요약**: 이 앱은 Claude Code CLI 로그인 세션을 그대로 쓰기 때문에 **Anthropic API 키는 필요 없습니다**. 아래 키들은 **모두 선택 사항**이며, 특정 모드/기능을 켤 때만 필요합니다. 하나도 없이 `start.command`를 실행해도 AI 회사·모의토론·비서·엔지니어링·데이터랩·미래상상 등 핵심 모드는 정상 동작합니다.
...
# 작업 완료 알림을 본인 텔레그램으로 받고 싶을 때 (5분 셋업)
TELEGRAM_BOT_TOKEN=
TELEGRAM_CHAT_ID=
...
#### 🟪 `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` — 완료 알림 텔레그램 봇

| | |
|---|---|
| **어떤 기능에?** | **자동실행·개발의뢰·플레이북(스킬)·미래상상·토론** 5개 모드의 작업 완료/실패/타임아웃 시 본인 텔레그램으로 알림 전송. 긴 작업을 걸어두고 다른 일을 할 때 유용 |
| **무엇인가?** | 본인이 만든 텔레그램 봇의 토큰 + 본인 개인 chat_id |
| **발급처** | 아래 3단계 |
| **비용** | **무료** |
| **발급 소요** | 약 5분 |
| **없으면?** | 알림만 조용히 건너뜁니다. 다른 모든 기능은 정상 동작. |

**셋업 3단계**

1. **봇 만들기** — 텔레그램 앱에서 `@BotFather` 검색 → `/newbot` → 봇 이름 입력 → 발급된 **토큰**을 `.env` 의 `TELEGRAM_BOT_TOKEN=` 뒤에 붙여넣기
2. **봇 활성화** — BotFather가 알려준 링크(`t.me/봇이름_bot`)로 방금 만든 봇을 열고 `/start` 한 번 전송. 텔레그램 스팸 방지 정책상 유저→봇 방향으로 먼저 말을 걸어야 봇이 회신 가능합니다.
3. **chat_id 확인** — 터미널에서
   ```bash
   python3 scripts/test_telegram.py --lookup
   # → chat_id=123456789 (본인이름)
   ```
   출력된 숫자를 `.env` 의 `TELEGRAM_CHAT_ID=` 뒤에 붙여넣기.

**테스트**
```bash
python3 scripts/test_telegram.py
# → 텔레그램에 "✅ [플레이북] 완료 — 텔레그램 연동 테스트" 메시지가 오면 성공
```

> 💡 토큰은 그대로 두고 알림만 일시적으로 끄려면 `TELEGRAM_NOTIFY_ENABLED=false` 로 바꾸세요. 실패·타임아웃도 이모지로 구분되어 옵니다 (✅ 성공 / ⚠️ 실패 / ⏱ 타임아웃). 사용자가 직접 "중지" 버튼을 눌러 취소한 작업은 알림이 오지 않습니다.
